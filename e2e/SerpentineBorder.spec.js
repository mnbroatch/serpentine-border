...
function queryString(params) {
  ...
  q.set('layout', params.layoutMode || 'content')
...
for (const params of PARAM_SETS) {
  test.describe(`SerpentineBorder (${params.label}: strokeWidth=${params.strokeWidth}, strokeCount=${params.strokeCount}, radius=${params.radius})`, () => {
    test('1. vertical segments overlap content side edges', async ({ page }) => {
      await page.goto(`/?${queryString({ ...params, layoutMode: 'content' })}`)
      ...
      // Some browsers report SVG bounding box clipped on the left; so assert total horizontal overflow and that SVG is wider than wrapper
    ...
    test('2. horizontal segments overlap space between children', async ({ page }) => {
      await page.goto(`/?${queryString({ ...params })}`)
      const wrapper = page.getByTestId('serpentine-wrapper')
    ...
    test('4. border layout mode: correct amount of space', async ({ page }) => {
      await page.goto(`/?${queryString({ ...params, layoutMode: 'border' })}`)
      const wrapper = page.getByTestId('serpentine-wrapper')
      await expect(wrapper).toBeVisible()
